...
var Global = {
	...
	*/
	//blm dites
	F4FilterOnGenerate: function(comp1, comp2,pTitleDlg, filter, filter2,value,colField, comp3, comp4, table) {
		console.log("masuk")
		if(comp3 == undefined) {
			comp3 = "" 
		}
		...
		var oTable = SAPUI.Table2("", "500px", 10, "Single", "Scroll")	
		oTable.setRowHeight(35);
		...
		var btnOK = SAPUI.Button("", "OK", "", "sap-icon://accept")
		var btnCancel = SAPUI.Button("", "Cancel", "", "sap-icon://sys-cancel")
		...
		$.ajax({
			...
			success: function (result) {  
				...
				if(ResultLength>0){
					...
					oModel.setData({WS_SP_DspValueHelpResult:tableContent}); 	
		...
		oTable.attachRowSelectionChange(function(oEvent) {  
			...
			//Tambahan untuk Set For All pada table
			var idx2 = table.getSelectedIndex();
			var cxt = table.getContextByIndex(idx2);
			var path = cxt.sPath;
			var obj = table.getModel().getProperty(path);

			if (obj.Derivative == "Set All") {
				var model = new sap.ui.model.json.JSONModel();
				var currentData = JSON.parse(table.getModel().getJSON())["rows"];
				...
				if (obj.Material != "") {
					for (var i = 0; i < currentData.length; i++) {
						currentData[i].Material = obj.Material;
					}			
				}

				if (obj.OrderType != "") {
					for (var i = 0; i < currentData.length; i++) {
						currentData[i].OrderType = obj.OrderType;
					}			
				}

				if (obj.Resource != "") {
					for (var i = 0; i < currentData.length; i++) {
						currentData[i].Resource = obj.Resource;
					}			
				}

				if (obj.Treatment != "") {
					for (var i = 0; i < currentData.length; i++) {
						currentData[i].Treatment = obj.Treatment;
					}			
				}

				if (obj.DimCore != "") {
					for (var i = 0; i < currentData.length; i++) {
						currentData[i].DimCore = obj.DimCore;
					}			
				}

				if (obj.Grade != "") {
					for (var i = 0; i < currentData.length; i++) {
						currentData[i].Grade = obj.Grade;
					}			
				}

				if (obj.Packing != "") {
					for (var i = 0; i < currentData.length; i++) {
						currentData[i].Packing = obj.Packing;
					}			
				}

				if (obj.LabelType != "") {
					for (var i = 0; i < currentData.length; i++) {
						currentData[i].LabelType = obj.LabelType;
					}			
				}

				if (obj.PackingCode != "") {
					for (var i = 0; i < currentData.length; i++) {
						currentData[i].PackingCode = obj.PackingCode;
					}			
				}

				model.setData({rows: currentData});
	...
	F4Filter: async function (comp1, comp2, pTitleDlg, filter, filter2, value, colField, comp3, comp4, table) {
	...
	F4FilterCustomer: async function (comp1, comp2, pTitleDlg, filter, filter2, value, colField, comp3, comp4, table, txfBind) {
		// Deklarasi objek untuk value help dialog
		// 1. Dialog
		var oDialog = SAPUI.Dialog("", "Select", "50%", "auto", true);
		// 2. Textview
		...
		oTable.attachRowSelectionChange(function (oEvent) {
			...
			if (this.isIndexSelected(idx)) {
				...
				var wsUrl = WS_SD + "WS_SKF1";

				var objParamSKF1 = {
					Customer: selectedItem,
				};

				var dynamicParamDev = Global.dynamicParam("DisplayCustomerAddress", objParamSKF1);
				...
				$.ajax({
					...
					success: function (result) {
						...
						if (result1[0][0].idMessage == 'SD002') {
							SAPUI.ErrorMessageDB(result1)
						} else {
							console.log("Bind: " + txfBind + ", Data: " + result1[0][0].Address)
							txfBind.setValue(result1[0][0].Address)
						}
					},
					error: function (jqXHR, textStatus, errorThrown) {
						loading.close()
			...
			success: function (result) {
				...
				if (deserilizeResult[0].length > 0) {
					for (var i = 0; i < deserilizeResult[0].length; i++) {
						...
						if (data == inputText.getValue()) {
							if (comp2 != "") {
								comp2.setText(obj[Object.keys(obj)[1]])
							}
							check++
						}
					}
	...
	/*f4 khusus kalau ngetik sendiri untuk isi dari dalam table
	 * belum di tes
	 */
	...
	F4FilterGetTextCustomer: function (comp1, comp2, inputText, filter, filter2, value, colField, txfBind) {
		// Deklarasi objek untuk value help dialog
		// 1. Dialog
		var oDialog = SAPUI.Dialog("", "Select", "50%", "auto", true);
		// 2. Textview
	...
	*/
	...
	F4FilterKolomDua: function (pTitleDlg, filter, filter2, value, colField, tableMain, pKeyBindKolom, idxSelectedRow, arrKolom, idxModel, pKeyBindKolomDua, arrKolomDua) {
		// Deklarasi objek untuk value help dialog
		// 1. Dialog
		var oDialog = SAPUI.Dialog("", "Select", "50%", "auto", true);
		// 2. Textview
			...
			if (this.isIndexSelected(idx)) {
				...
				for (j = 0; j < pKeyBindKolom.length; j++) {
					for (i = 0; i < data.length; i++) {
						if (i == idxSelectedRow) {
							data[i][arrKolom[j]] = obj[pKeyBindKolom[j]]
							data[i][arrKolomDua[j]] = obj[pKeyBindKolomDua[j]]
						}
					}
				}
				tableMain.getModel().refresh()
	...
	Numeric: function (comp) {
		...
		if (!tfValue.match(myInteger)) {
			...
			comp.setValueStateText("Fill this correctly");
	...
	NumericOnlyWithComma: function (pComp) {
		...
		if (!isValid) {
			...
			pComp.setValueStateText("Fill this correctly");
			return false;
		} else {
	...
	cekDatePicker2: function (pID) {
		...
		pID.attachValidationError(
			function (oEvent) {
				...
				oElement.setValueStateText("Fill this correctly")
	...
	cekDatePicker3: function (pID) {
		...
		if (!pID.getValue().match(regExMatchFormat)) {
			...
			SAPUI.ErrorMessageView("E0082", "")

			pID.setValueState("Error")
			pID.setValueStateText("Fill this correctly")
			return false;
		}
		else if (!pID._bValid) {
			SAPUI.ErrorMessageView("E0102", "")
	...
	cekDatePicker5: function (pIDFrom, pIDTo, oEvent) {
		...
		if (date2 < date1) {
			...
			oEvent.oSource.setValueStateText("Fill this correctly");
			return false;
		} else {
			//				SAPUI.ErrorMessageView("","");
			oEvent.oSource.setValueState(sap.ui.core.ValueState.None);
	...
	UniversalBack: function () {
		...
		//Custom button back
		//Karena QGR3.html memiliki link ke QGR1.html dan QGR2.html
		var pageBefore = "" + document.referrer.split("/").slice(-1);
		var pageNow = "" + location.href.split("/").slice(-1);
		
		if (pageNow.includes("html")) {
			//Harus membandingkan 3 huruf terdepan dari halaman saat ini dan sebelumnya
			//Karena mayoritas yang mempunyai link akan memiliki 3 huruf yang sama di awal
			if (pageBefore.substring(0, 2) == pageNow.substring(0, 2)) {
				var pageNow2 = pageNow.split(".");
				//Jika yang dibuka bukan halaman display (tidak memiliki angka 3 di tcode-nya)
				if (pageNow2[0].substring(pageNow2[0].length - 1) != 3) {
					window.history.back()		
				} else { //Jika yang dibuka halaman display
	...
	Logout: async function () {
		...
		var paramUser = {
			UsernameIn: window.localStorage.getItem("userNameLS"),
			ip: "",//window.localStorage.getItem("ActivityIP"),
			server: Server1,
			hostname: "",
			today: today,
			time: time
		...
		var req = {
			url: WS_User + "WS_UC_Auth",
			method: "Logout",
			body: paramUser,
		}

		try {
			if (paramUser.UsernameIn != null) {
				data = await fetchApi(req)
				console.log(data)
			}
			Server1 = "none";
			window.localStorage.clear();
	...
	ChangeFormatDateToDot: function (value) {
		var splitValue = value.split('T')[0].split('-')
		//console.log(splitValue)
		rearrange = [splitValue[2], splitValue[1], splitValue[0]].join('.')
		return rearrange
	},
	...
	ChangeFormatDateValueMonth: function (pValue) {
		var rearrange = "";
		if (pValue == "") {
			rearrange = "";
		} else {
			var splitValue = pValue.split('.');
			rearrange = [splitValue[1], splitValue[0], '01'].join('-')
		}
		return rearrange;
	},
	...
	isParameterToValid: function (mode, pComponen1, pComponen2) {
		...
		if (pComp1 != "" && pComp2 != "") {
			...
			if (array.indexOf(pComp1) > array.indexOf(pComp2)) {
				SAPUI.ErrorMessageView("SY014", "")
				pComponen1.setValueState(sap.ui.core.ValueState.Error)
				pComponen1.setValueStateText("Fill this correctly");
				pComponen2.setValueState(sap.ui.core.ValueState.Error)
				pComponen2.setValueStateText("Fill this correctly");
				return false
			}
		}
	...
	isDecimal: function (pComponent) {
		...
		if (!(input.match(expression))) {
			...
			pComponent.setValueStateText("Fill this correctly");
			return false;
		}
		return true;
	},
	...
	dynamicDeserialize: function (ajaxResult) {
		...
		if ("exceptionErrorStatus" in tempResult) {
			...
			var message = tempResult.message
			/* SAPUI.MessageDialog({
				Type: "E",
				Message: message,
			}) */
			...
			SAPUI.DialogErrMessage(result, 'previousPage')
			return retVal;
		}
	...
	setSessionStorage: function () {
		...
		window.sessionStorage.setItem("paramOrder26", pOrderList)
	},
	...
	InsLogActivity: async function (searchTcode, msg = "") {
		var today = this.getNow().today
		var time = this.getNow().time
		var username = window.localStorage.getItem('userNameLS');
		...
		if (msg == "") {
			msg = `Transaction ${searchTcode} Started`
		}
		...
		try {
			data = await fetchApi(req)
			window.sessionStorage.setItem("tcodeVariantux5", searchTcode);
	...
	GetAuthFieldTcode: function (Tcode, Object, Value) {
	...
	onConvertNumber: function(x, addComma) {
		var value = x.toString().split('.')
		...
		if (addComma) {
			//addComma
			value = numerator.replace(/\B(?=(\d{3})+(?!\d))/g, ",") + denominator;
		} else {
			//removeComma
			value = parseFloat(numerator.replace(/,/g, '').trim() + denominator);
		}
	...
	IsMaintenance: function() {
		...
		var aData = {};
		var dynamicParam = Global.dynamicParam("DspMaintenanceStatus", aData);
		...
		$.ajax({
			...
			success: function(result) {
				var result1 = Global.dynamicDeserialize(result);
				var temp = result1[0][0].CharacteristicValue
